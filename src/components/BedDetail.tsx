...
export default function BedDetail({ cama, onClose, onOpenMonitoringForm }: BedDetailProps) {
  if (!cama) {
    return (
      <div className="bg-slate-50 border border-slate-100 rounded-2xl p-8 flex flex-col items-center justify-center text-center h-full min-h-[400px]">
        <div className="w-16 h-16 bg-white border border-slate-200 rounded-2xl flex items-center justify-center text-slate-400 shadow-xs mb-4">
          <Info className="w-8 h-8 text-slate-400" />
        </div>
        <h3 className="font-display font-medium text-slate-700 text-base">
          Ninguna Cama Seleccionada
        </h3>
        <p className="text-sm text-slate-400 max-w-[280px] mt-2">
          Haz clic en cualquier celda de cama en el mapa espacial para desplegar su expediente agronómico completo.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white border border-slate-100 rounded-2xl shadow-sm p-6 flex flex-col h-full overflow-y-auto" id="bed-expediente-panel">
        ...
        <div>
          <div className="flex items-center gap-2 mb-1">
            <span className="text-xs font-semibold font-mono text-slate-400 uppercase">
              Bloque {cama.bloqueId}
            </span>
            <span>•</span>
            <span className="text-xs bg-slate-100 px-2 py-0.5 text-slate-600 rounded font-mono font-medium">
              Cama {cama.numero}
            </span>
          </div>
          ...
          <p className="text-xs text-slate-500 font-mono">
            Variedad: <strong className="text-slate-700">{cama.produccion.variedad}</strong>
          </p>
        </div>
      ...
      <button
        ...
        className="w-full flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white py-2.5 px-4 rounded-xl text-sm font-medium transition-colors shadow-sm hover:shadow-xs cursor-pointer mb-5"
        ...
        <div className="bg-slate-50/50 rounded-xl p-4 border border-slate-100">
          ...
          <div className="mt-3">
            <div className="flex justify-between text-xs mb-1">
              ...
              <span className="font-semibold text-slate-800 font-mono">{cama.mipe.incidencia}%</span>
            </div>
            <div className="w-full bg-slate-200 rounded-full h-2">
              <div 
                className={`h-2 rounded-full ${cama.mipe.incidencia > 20 ? 'bg-red-500' : cama.mipe.incidencia > 10 ? 'bg-orange-400' : 'bg-emerald-500'}`}
                style={{ width: `${cama.mipe.incidencia}%` }}
              ></div>
            </div>
            <p className="text-[10px] text-slate-400 mt-1 font-mono">
              Umbral máximo aceptable: {RANGOS_OPTISMOS.mipe.incidenciaMaximaAceptable}% de aspersión
            </p>
          </div>
        </div>
