...
export default function FarmMap({
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 flex flex-col h-full" id="agricultural-spatial-map">
      {/* Cabecera del Mapa */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          <h2 className="text-xl font-display font-semibold text-slate-800 flex items-center gap-2">
            <Activity className="w-5 h-5 text-emerald-500" />
            Mapa Físico de Camas por Bloque
          </h2>
          <p className="text-sm text-slate-500">
            Vista espacial integrada. Haz clic en cualquier cama para analizar sus variables agronómicas correlacionadas.
          </p>
        </div>

        {/* Indicador de Capa */}
        <div className="flex items-center gap-2 bg-slate-50 border border-slate-100 px-3 py-1.5 rounded-full text-xs text-slate-600 self-start">
          <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>
          Capa: <strong className="capitalize text-slate-800 font-medium">
            {activeLayer === 'riesgo' && 'Riesgo Integrado'}
            {activeLayer === 'mipe' && 'Fitosanitario (MIPE)'}
            {activeLayer === 'mirfe' && 'Fertirriego (MIRFE)'}
            {activeLayer === 'produccion' && 'Productividad Semanal'}
          </strong>
        </div>
      </div>
